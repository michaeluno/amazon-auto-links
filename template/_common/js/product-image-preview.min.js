/* Image Preview Tooltip 1.0.1 */
(function($){var imageSize=420;$(document).ready(function(){setMainThumbnailDataAttributes(this);initialize('.amazon-auto-links-product-image img[data-large-src]');});$('body').on('aal_ajax_loaded_unit',function(event){setMainThumbnailDataAttributes(event.target);initialize($(event.target).find('.amazon-auto-links-product-image img[data-large-src]'));});function setMainThumbnailDataAttributes(target){$(target).find('.amazon-product-thumbnail-container[data-large-src]').each(function(){$(this).find('.amazon-product-thumbnail img').attr('data-large-src',$(this).attr('data-large-src')).attr('href',$(this).attr('data-href'));});}
