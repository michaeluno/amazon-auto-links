...
function resizedWindow(event){var _iOverlap=_getSideMetaBoxOverlapWidth();if(!_iOverlap){_oChart.canvas.parentNode.style.width='auto';return;}
...
function getChartCreated(oChartElement,aX,aY,iTotal){return new Chart(oChartElement,{"type":"line","data":{"labels":aX,"datasets":[{"label":aalChartJSLoader.labels.counts+' ('+aalChartJSLoader.labels.total+': '+iTotal+')',"data":aY,"fill":false,"borderColor":"rgb(75, 192, 192)","lineTension":0}]},"options":{responsive:true,maintainAspectRatio:true,legend:{display:true,},tooltips:{callbacks:{label:function(tooltipItem){return tooltipItem.yLabel;}}},scales:{xAxes:[{type:'time',time:{unit:'day',displayFormats:{day:'YYYY-MMM-D',},tooltipFormat:'ll',},ticks:{callback:_formatDateLabel,source:'labels'},scaleLabel:{display:true,labelString:aalChartJSLoader.labels.dates}}],yAxes:[{scaleLabel:{display:true,labelString:aalChartJSLoader.labels.counts},ticks:{suggestedMin:0}}]}}});}
