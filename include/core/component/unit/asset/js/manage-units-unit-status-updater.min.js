...
_oPendingUnits.each(function(index,element){if($(element).hasClass('done')){return true;}
$(element).data().countTrial++;if($(element).data('countTrial')>3){$(element).addClass('gray done');return true;}
var _sUnitID=$(element).attr('data-post-id');$(element).removeClass('gray').addClass('loading');jQuery.ajax({type:"post",dataType:'json',url:aalUnitStatusUpdater.ajaxURL,data:{action:'aal_action_update_unit_status',aal_nonce:$('#amazon-auto-links-nonce').val(),units:[_sUnitID]},success:function(response){if(response.success){$.each(response.result,function(sIndex,value){if('normal'===value){$('.unit-status[data-post-id='+sIndex+']').removeClass('loading').addClass('green done');return true;}
if(value){$('.unit-status[data-post-id='+sIndex+']').removeClass('loading').addClass('red done');return true;}});}else{}},complete:function(){if($(element).hasClass('done')){return;}
