...
var _oOutput2=_oOutput.clone();_oOutput.css({'position':'relative','left':-99999,'top':-99999,'float':'left','display':'block',}).appendTo(_oElement.parent());_oElement.removeClass('now-retrieving');setTimeout(function(){var _iHeight=_oOutput[0].offsetHeight;_oOutput.remove();_oElement.wrap('<div></div>');_oElement.parent().css({'min-height':_iHeight+'px',});_oOutput2.css({'opacity':'0'});_oElement.replaceWith(_oOutput2);_oOutput2.unwrap();_oOutput2.animate({'opacity':'1'});},1000);});}else{}},error:function(response){},complete:function(){_oSpinner.remove();setTimeout(function(){processNowRetrieving(subject);},3000);}});}
