...
debugLog(aalButtonPreview);var _oButtonSelect=$('.button-select-row').find('select');_oButtonSelect.on('change',function(){_setPreviewButton($(this).val(),$(this));});_oButtonSelect.trigger('change');$('input.override-button-label[type=checkbox]').on('change',function(){if(!$(this).is(':checked')){___revertButtonLabels(_oButtonSelect);return;}
debugLog('The Override Label option is on.');___setButtonLabelByInput($(this).closest('.amazon-auto-links-section-table').find('input.button-label[type=text]').first());});$('input.button-label[type=text]').on('change',function(){var _oOverrideLabel=$(this).closest('.amazon-auto-links-section-table').find('input.override-button-label[type=checkbox]');if(!_oOverrideLabel.is(':checked')){return;}
debugLog('The Override Label option is on.');___setButtonLabelByInput(this);});$('.iframe-button-preview-container').find('iframe').on('load',function(){$('input.override-button-label[type=checkbox]').trigger('change');});function _setPreviewButton(iButtonID,oSelect){iButtonID=parseInt(iButtonID);var _oButton=oSelect.closest('fieldset').find('.amazon-auto-links-button');if('undefined'===typeof aalButtonPreview.activeButtons[iButtonID]){debugLog.log('The button label does not exist. button ID:',iButtonID,'Active buttons:',aalButtonPreview.activeButtons);return;}
var _oButtonContainer=$(_oButton).parent();var _sButtonLabel=oSelect.closest('.amazon-auto-links-section-table').find('input.override-button-label[type=checkbox]').is(':checked')?oSelect.closest('.amazon-auto-links-section-table').find('input.button-label[type=text]').val():aalButtonPreview.activeButtons[iButtonID];debugLog('Setting the button label: ',_sButtonLabel,' ID: ',iButtonID);if(iButtonID){$(_oButton).attr('class','amazon-auto-links-button amazon-auto-links-button-'+iButtonID);___setButtonLabel_div(_sButtonLabel,oSelect);_oButtonContainer.show();_oButtonContainer.siblings('.iframe-button-preview-container').css({'position':'absolute','top':'-9999px','z-depth':'-100',});return;}
_oButtonContainer.hide();___setButtonLabel_iframe(_sButtonLabel,oSelect);_oButtonContainer.siblings('.iframe-button-preview-container').css({'position':'static','top':'0','z-depth':'1',});}
function ___revertButtonLabels(oButtonSelect){___setButtonLabel_iframe(aalButtonPreview.activeButtons[0],oButtonSelect);var _iButtonID=parseInt(oButtonSelect.val());if(_iButtonID&&aalButtonPreview.activeButtons[_iButtonID]){___setButtonLabel_div(aalButtonPreview.activeButtons[_iButtonID],oButtonSelect);}
