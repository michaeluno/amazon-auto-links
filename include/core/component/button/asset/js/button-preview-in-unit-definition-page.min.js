...
if(debugMode){console.log('Amazon Auto Links Button Preview Script',aalButtonPreview);}
var _setPreviewButton=function(iButtonID,oSelect){iButtonID=parseInt(iButtonID);var _oButton=oSelect.closest('fieldset').find('.amazon-auto-links-button');if('undefined'===typeof aalButtonPreview.activeButtons[iButtonID]){if(debugMode){console.log('the button label does not exists. button ID',iButtonID,'active buttons',aalButtonPreview.activeButtons);}
return;}
...
_oButtonContainer.hide();___setButtonLabel_iframe(_sButtonLabel,oSelect);_oButtonContainer.siblings('.iframe-button-preview-container').css({'position':'static','top':0,'z-depth':1,});};var _oButtonSelect=$('.button-select-row').find('select');_oButtonSelect.change(function(){_setPreviewButton($(this).val(),$(this));}).trigger('change');$('input.override-button-label[type=checkbox]').change(function(){if(!$(this).prop('checked')){___revertButtonLabels(_oButtonSelect);return;}
___setButtonLabelByInput($(this).closest('.amazon-auto-links-section-table').find('input.button-label[type=text]').first());});$('input.button-label[type=text]').change(function(){var _oOverrideLabel=$(this).closest('.amazon-auto-links-section-table').find('input.override-button-label[type=checkbox]');if(!_oOverrideLabel.prop('checked')){return;}
___setButtonLabelByInput(this);});$('.iframe-button-preview-container').find('iframe').on('load',function(){$('input.override-button-label[type=checkbox]').trigger('change');});function ___revertButtonLabels(oButtonSelect){___setButtonLabel_iframe(aalButtonPreview.activeButtons[0],oButtonSelect);$.each(aalButtonPreview.activeButtons,function(__iButtonID,__sButtonLabel){if(0===__iButtonID){return true;}
...
aalSetButtonPreviewIframeStyle(_oButton[0].offsetWidth,_oButton[0].offsetHeight);}});}(jQuery));;