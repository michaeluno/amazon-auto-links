/* Button Preview Framed Page 1.1.1 */
(function(){'use strict';if(document.querySelector){if(!window.addEventListener){return}}
var lastHeight;document.addEventListener('ReloadButtonPreview',function(e){console.log('Framed event:',e);var _previewInfo=getPreviewInformation();if(lastHeight===_previewInfo.height){return}
...
var _element=document;var _e;if(document.createEvent){_e=document.createEvent('HTMLEvents');_e.initEvent(event.data.event,!0,!0);_e._eName=event.data.event;_element.dispatchEvent(_e)}else{_e=document.createEventObject();_e._eName=event.data.event;_e._eType=event.data.event;_element.fireEvent('on'+_e._eType,_e)}},!1);window.addEventListener('DOMContentLoaded',function(e){setParentButtonPreviewIframeStyle()});function setParentButtonPreviewIframeStyle(){if('undefined'===typeof parent.aalSetButtonPreviewIframeStyle){return}
