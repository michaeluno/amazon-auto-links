...
var iframes=document.querySelectorAll('iframe.components-sandbox, iframe[data-secret="'+data.secret+'"]'),i,_thisFrame;for(i=0;i<iframes.length;i++){_thisFrame=iframes[i];if(e.source!==_thisFrame.contentWindow&&e.source.parent!==_thisFrame.contentWindow){continue;}
...
function adjustFrameHeight(event){var _attrID=event.detail.id;var _iHeight=event.detail.height;event.detail.source.setAttribute('id',_attrID);event.detail.source.setAttribute('height',_iHeight.toString());event.detail.source.setAttribute('scrolling','no');var _secret=event.detail.data.secret;var _css='#'+_attrID+' { height: '+_iHeight+'px; min-height: '+_iHeight+'px; overflow: hidden;  } ';if(event.detail.isSandbox){var _innerIframe=event.detail.source.contentWindow.document.querySelector('iframe[data-secret="'+_secret+'"]');if(null!==_innerIframe){_innerIframe.setAttribute('height',_iHeight.toString());}}
var _styleElem=document.getElementById('aalEmbed-style-'+_secret);if(_styleElem){_styleElem.innerHTML=_css;return;}
