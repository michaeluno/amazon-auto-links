...
(function($){$(document).ready(function(){ajaxManager.run();$('.aal-tests').click(function(event){event.preventDefault();$('.item-select-error').remove();var _oCheckedTags=$('input.test-tags:checked');if(!_oCheckedTags.length){$(this).parent().parent().append('<span class="item-select-error">* Please select items.</span>');return false;}
_oCheckedTags.each(function(){var _sLabel=$(this).closest('label').text();if('undefined'===typeof aalTests.files[_sLabel]){console.error('Files for '+_sLabel+' could not be found.');return true;}
$('*[class*="'+_sLabel+'"]').remove();var _oSection=$('<div class="'+_sLabel+'"></div>');$('.results').append(_oSection);_oSection.accordion({collapsible:true,active:false,heightStyle:"content"});var _aFiles=aalTests.files[_sLabel];$.each(_aFiles,function(index,sFilePath){___testFile(_sLabel,sFilePath);});});return false;});function ___testFile(sLabel,sFilePath){var _oStartButton=$('.aal-tests');ajaxManager.addRequest({type:"post",dataType:'json',async:true,url:aalTests.ajaxURL,data:{action:aalTests.actionHookSuffix,aal_nonce:aalTests.nonce,file_path:sFilePath,},spinnerImage:$('<img src="'+aalTests.spinnerURL+'" />'),startButton:_oStartButton,buttonLabel:_oStartButton.val(),resultsArea:$('*[class*="'+sLabel+'"]'),beforeSend:function(){this.spinnerImage.css({'vertical-align':'middle','display':'inline-block','height':'auto','margin-left':'0.5em'});this.startButton.val('Running...');this.startButton.parent().parent().append(this.spinnerImage);},success:function(response){___setResponseOutput(this.resultsArea,response,sFilePath);},error:function(response){this.resultsArea.append('<div class="response-error">'
