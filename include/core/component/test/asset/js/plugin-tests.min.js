...
var _sResultClass='result-'+_sLabel;$('*[class*="'+_sResultClass+'"]').remove();var _oSection=$('<div class="'+_sResultClass+'"></div>');$('.results').append(_oSection);var _aFiles=aalTests.files[_sLabel];$.each(_aFiles,function(index,sFilePath){$('*[class*="'+_sResultClass+'"]').html('');___runFile(_sLabel,sFilePath,_aTags);});});return false;});function ___runFile(sLabel,sFilePath,aTags){var _oStartButton=$('.aal-tests');ajaxManager.addRequest({type:"post",dataType:'json',async:true,url:aalTests.ajaxURL,data:{action:aalTests.actionHookSuffix,aal_nonce:aalTests.nonce,file_path:sFilePath,tags:aTags,},spinnerImage:$('<img src="'+aalTests.spinnerURL+'" alt="Now loading..." />'),startButton:_oStartButton,buttonLabel:_oStartButton.val(),resultsArea:$('*[class*="'+'result-'+sLabel+'"]'),beforeSend:function(){this.spinnerImage.css({'vertical-align':'middle','display':'inline-block','height':'auto','margin-left':'0.5em'});this.startButton.val('Running...');this.startButton.parent().parent().append(this.spinnerImage);},success:function(response){___setResponseOutput(this.resultsArea,response,sFilePath);},error:function(response){this.resultsArea.append('<div class="response-error">'
...
+response.result
+'</p>');return;}
$.each(response.result,function(sIndex,eachResult){var _sHeader='';var _sDetails='';var _sPurpose='';var _sCurrentItem='<p>File: '+sFilePath+'</p>';var _iLine=eachResult.line?eachResult.line:0;if(_iLine){_sCurrentItem=_sCurrentItem+'<p>Line: '+_iLine+'</p>';}
