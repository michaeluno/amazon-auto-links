...
initialize(this,options)};function initialize(target,options){var _this=$(target);var _content=$(target).attr('data-tooltip-content');_content=_content?_content:undefined;options='undefined'===typeof options?{}:options;options=$.extend({},{pointerClass:'amazon-auto-links-tooltip-survey-balloon',width:$(target).data('width')||options.width||340,shown:false,content:_content,oneOff:false,autoClose:true,noArrow:false,},options);options.pointerClass+=options.noArrow?' no-arrow':'';$(_this).removeClass('no-js');if(options.shown){handleTooltip(target,options);}
...
function isHTML(str){var a=document.createElement('div');a.innerHTML=str;for(var c=a.childNodes,i=c.length;i--;){if(1===c[i].nodeType){return true;}}
return false;}}(jQuery));;