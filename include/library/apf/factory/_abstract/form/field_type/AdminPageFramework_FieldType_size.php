...
class AmazonAutoLinks_AdminPageFramework_FieldType_size extends AmazonAutoLinks_AdminPageFramework_FieldType_select {
    ...
    protected function getStyles() {
        return <<<CSSRULES
/* Size Field Type */
.amazon-auto-links-field-size input {
    text-align: right;
}
.amazon-auto-links-field-size select.size-field-select {
    vertical-align: 0px;     
}
.amazon-auto-links-field-size label {
    width: auto;     
} 
.form-table td fieldset .amazon-auto-links-field-size label {
    display: inline;
}
CSSRULES;
    ...
    protected function _getFieldOutputByLabel($isKey, $sLabel, array $aField) {
        ...
        $_aOutput = array($this->getElementByLabel($aField['before_label'], $isKey, $_bMultiLabels), "<div class='amazon-auto-links-input-label-container amazon-auto-links-select-label' style='min-width: " . $this->sanitizeLength($aField['label_min_width']) . ";'>", $this->_getNumberInputPart($aField, $_aBaseAttributes, $isKey, $_bMultiLabels), $this->_getUnitSelectInput($aField, $_aBaseAttributes, $isKey, $_bMultiLabels), "</div>", $this->getElementByLabel($aField['after_label'], $isKey, $_bMultiLabels));
        return implode('', $_aOutput);
    }
    private function _getNumberInputPart(array $aField, array $aBaseAttributes, $isKey, $bMultiLabels) {
        ...
        return "<label " . $this->getAttributes($_aSizeLabelAttributes) . ">" . $this->getElement($aField, $bMultiLabels ? array('before_label', $isKey, 'size') : array('before_label', 'size')) . ($aField['label'] && !$aField['repeatable'] ? "<span class='amazon-auto-links-input-label-string' style='min-width:" . $this->sanitizeLength($aField['label_min_width']) . ";'>" . $_sLabel . "</span>" : "") . "<input " . $this->getAttributes($_aSizeAttributes) . " />" . $this->getElement($aField, $bMultiLabels ? array('after_input', $isKey, 'size') : array('after_input', 'size')) . "</label>";
    }
