...
.amazon-auto-links-field-color .wp-picker-container {
    vertical-align: middle;
}
.amazon-auto-links-field-color .ui-widget-content {
    border: none;
    background: none;
    color: transparent;
}
.amazon-auto-links-field-color .ui-slider-vertical {
    width: inherit;
    height: auto;
    margin-top: -11px;
}
...
jQuery( document ).ready( function(){
    ...
    jQuery().registerAmazonAutoLinks_AdminPageFrameworkCallbacks( {     
        added_repeatable_field: function( node, sFieldType, sFieldTagID, sCallType ) {
            ...
            registerAmazonAutoLinks_AdminPageFrameworkColorPickerField( nodeNewColorInput );     
...
JAVASCRIPTS;
    ...
    protected function getField($aField) {
        ...
        return $aField['before_label'] . "<div class='amazon-auto-links-input-label-container'>" . "<label for='{$aField['input_id']}'>" . $aField['before_input'] . ($aField['label'] && !$aField['repeatable'] ? "<span class='amazon-auto-links-input-label-string' style='min-width:" . $this->sanitizeLength($aField['label_min_width']) . ";'>" . $aField['label'] . "</span>" : "") . "<input " . $this->getAttributes($aField['attributes']) . " />" . $aField['after_input'] . "<div class='repeatable-field-buttons'></div>" . "</label>" . "<div class='colorpicker' id='color_{$aField['input_id']}'></div>" . $this->_getColorPickerEnablerScript("{$aField['input_id']}") . "</div>" . $aField['after_label'];
    }
